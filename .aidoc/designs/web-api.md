...
Action requests contain `expected_revision`, an action kind, and only the fields required by that action. The API translates accepted action kinds to `game.Action`; unknown kinds or extra fields fail before the engine is called. The `set-notes` action directly replaces one cell's complete manual-note set. The separate `adopt-candidates-as-notes` action is a revision-checked whole-board transition: the engine derives current candidates, replaces notes on every editable visibly empty cell, and applies the initiating digit toggle as one mutation and one history entry. Clients never copy or submit derived candidate grids. Presentation preferences such as theme, focus, or candidate visibility remain client-owned.
