...
func solve(board *core.SudokuBoard, state *solveState, options solveOptions) bool {
	for _, row := range options.RowOrder {
		for _, column := range options.ColumnOrder {
			cell := core.NewCell(row, column)

			if board.Get(cell) == 0 {
				// When counting solutions, we do not need to generate candidate numbers randomly
				candidateNumbers := generateNumberArray(1, 10, !options.CountSolutions && options.Randomly)
...
// Function to solve the Sudoku board with random candidate numbers
